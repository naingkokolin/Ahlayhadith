...
const HadithBooksPage: React.FC<Props> = ({ books, onAdd, onUpdate, onDelete }) => {
  ...
  const openEdit = (b: IHadithBook) => {
    ...
    setForm({ book_number: b.book_number, name_ar: b.name_ar, name_mm: b.name_mm, name_en: b.name_en, author: b.author, totalHadith: b.totalHadith });
    setFormError(""); setModalOpen(true);
  };

  const handleSave = () => {
    if (!form.book_number || !form.name_ar || !form.name_en || !form.name_mm || !form.author) {
  ...
  return (
    <div>
      ...
      <div style={styles.content}>
        <div className="table-card">
          <div className="table-header">
            <div className="table-title">All Books <span className="badge badge-gold" style={{ marginLeft: 8 }}>{books.length}</span></div>
            <input className="search-input" placeholder="Search book..." value={search}
              onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
          </div>
          <table>
            <thead>
              <tr>
                <th>#</th><th>Arabic</th><th>English</th><th>Myanmar</th><th>Author</th><th>Hadiths</th><th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {slice.length === 0 ? (
                <tr><td colSpan={7} style={styles.empty}>No books found</td></tr>
              ) : slice.map((b) => (
                <tr key={b._id}>
                  <td><span className="badge badge-gold">{b.book_number}</span></td>
                  ...
                  <td>
                    <div className="action-cell">
                      ...
                      <button className="btn btn-sm btn-danger" onClick={() => { setDelTarget(b); setDelOpen(true); }}>Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)}
        ...
        footer={<><button className="btn" onClick={() => setModalOpen(false)}>Cancel</button><button className="btn btn-primary" onClick={handleSave}>Save Book</button></>}
      >
        <div style={styles.formGrid}>
          <div className="field-wrap">
            <label className="field-label">Book Number</label>
            <input className="field-input" type="number" min={1} value={form.book_number || ""}
              onChange={(e) => set("book_number", parseInt(e.target.value))} placeholder="e.g. 1" />
          </div>
          <div className="field-wrap">
          ...
          <div className="field-wrap" style={{ gridColumn: "span 2" }}>
            ...
            <input className="field-input" type="text" value={form.name_ar}
              onChange={(e) => set("name_ar", e.target.value)} placeholder="e.g. صحيح البخاري"
          ...
          <div className="field-wrap">
            ...
            <input className="field-input" type="text" value={form.name_en}
              onChange={(e) => set("name_en", e.target.value)} placeholder="e.g. Sahih al-Bukhari" />
          </div>
          <div className="field-wrap">
      ...
      {/* Delete Modal */}
      <Modal open={delOpen} onClose={() => setDelOpen(false)} title="Delete Book" danger width={360}
      ...
      >
        <div style={{ textAlign: "center", padding: "8px 0 16px" }}>
          ...
          <div style={{ fontSize: 13, color: "var(--text2)" }}>
            Delete <strong style={{ color: "var(--text)" }}>{delTarget?.name_en}</strong>?<br />All associated chapters and hadiths may be affected.
          </div>
        </div>
      </Modal>
    </div>
